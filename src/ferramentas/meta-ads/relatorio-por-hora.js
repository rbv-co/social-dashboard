...
// no mesmo dia (pedido do dono: "tira o link_click, apenas visitas no perfil
// e seguidores") — não existe por campanha pra nenhum dos dois (Meta não
// atribui nem seguidor nem visita a uma campanha específica), então não tem
// por que fingir granularidade que não existe.
//
// `seguidoresTotal` decide se o bloco de seguidor aparece (vem de
// `seguidoresTotalNaHora`) — é o mais permissivo dos três: a primeira
// leitura da série tem total mas não tem delta de período (pedido do dono,
// 12/09/2026: "coloca o total de seguidores também" e depois "novos
// seguidores no período / total do dia / total da conta"). `null` em cada
// valor = sem leitura pra essa hora (ou pro dia), não entra na mensagem.
